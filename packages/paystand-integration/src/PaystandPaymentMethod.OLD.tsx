...
const PaystandPaymentMethod: FunctionComponent<PaymentMethodProps> = ({
    ...
    const apiService = useMemo(
        () => {
            ...
            // 'development' for .io, 'biz' for production (API uses 'development' for staging/io)
            const environment = state.config.useSandbox === 0 ? 'biz' : 'development';
    ...
    const handleTokenizePayment = useCallback(async () => {
        try {
            ...
            // Environment mapping:
            // 'staging' -> uses .io domain (like Magento)
            // 'development' -> uses .biz domain  
            // 'production' -> uses .biz/com domain
            // Default to 'staging' if useSandbox is undefined or 1
            ...
            console.log('⚠️ Using "staging" for .io domain (like Magento)');
            ...
            const setupPayStandHandlers = () => {
                ...
                const checkForPayStand = (attempts = 0) => {
                    ...
                    if ((window as any).PayStandCheckout) {
                        ...
                        PayStandCheckout.onComplete(async (data: any) => {
                            console.log('═══════════════════════════════════════════════════');
                            ...
                            // 🚀 AUTO-SUBMIT ORDER AFTER SUCCESSFUL PAYMENT
                            try {
                                console.log('═══════════════════════════════════════════════════');
                                console.log('🚀 AUTO-SUBMITTING ORDER AFTER PAYSTAND COMPLETION');
                                console.log('═══════════════════════════════════════════════════');
                                
                                // Submit order with moneyorder method
                                const orderResult = await checkoutService.submitOrder({
                                ...
                                console.log('📦 Full order state:', JSON.stringify(order, null, 2));
                                
                                // Check if we have an order ID and redirect
                                if (order && order.orderId) {
                                    console.log('🎯 Order ID found:', order.orderId);
                                    
                                    // Construct the order confirmation URL
                                    const orderConfirmationUrl = `/checkout/order-confirmation/${order.orderId}`;
                                    
                                    console.log('🔄 Redirecting to:', orderConfirmationUrl);
                                    
                                    // Force redirect to thank you page
                                    window.location.href = orderConfirmationUrl;
                                } else {
                                    console.warn('⚠️ Order created but no orderId found in response');
                                }
                                ...
                                setState((prev) => ({
                                    ...prev,
                                    error: 'Error al procesar la orden: ' + (error instanceof Error ? error.message : 'Unknown error'),
                                    isTokenizing: false,
                                }));
